...
BluezManager.prototype.init = function init(cb) {
    ...
    self._crawl(function(err) {
        ...
        self._advertise(function(err) {
            //TODO for now igorning adveritising errors until I find a robust way of handling it.
            if (err) {
                debug('Advertising error', err);
                return;
            }
            debug('Advertising successful');
        });
